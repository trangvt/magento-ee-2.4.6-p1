...
class Builder
{
     ...
     * @param WishlistProviderInterface|null $wishlistProvider
     */
    public function __construct(
        ...
        WishlistProviderInterface $wishlistProvider = null
    ) {
        ...
        $this->wishlistProvider = $wishlistProvider ?: ObjectManager::getInstance()->get(
    ...
    public function build(array $buyRequest, $itemId, $allowMisconfiguredProducts)
    {
        ...
        $isFromWishlist = isset($buyRequest['from_wishlist']) && in_array('save_old', $buyRequest);
        unset($buyRequest['from_wishlist']);
        $params = [];
        if ((int)$itemId) {
        ...
        $configuredOptions = $buyRequest->getData('options') ? $buyRequest->getData('options') : [];

        if ($isFromWishlist) {
            $wishlist = $this->wishlistProvider->getWishlist();
            $buyRequest = $wishlist->getItem($buyRequest->getData('item'))->getBuyRequest();
            $wishlistOptions = $buyRequest->getData('options');
            foreach ($configuredOptions as $key => $value) {
                if ($value !== $wishlistOptions[$key]) {
                    $wishlistOptions[$key] = $value;
                }
            }
            $buyRequest->setData('options', $wishlistOptions);
        }
