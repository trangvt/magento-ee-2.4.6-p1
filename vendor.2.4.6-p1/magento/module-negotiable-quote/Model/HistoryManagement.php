...
class HistoryManagement implements HistoryManagementInterface
{
    ...
    public function updateLog($quoteId, $isSeller = false, $status = '')
    {
        ...
        if ($lastLog) {
            $lastLogData = $this->serializer->unserialize($lastLog->getLogData());
            $lastLogDataStatus = $lastLogData['status']['new_value'] ?? null;

            if ($lastLogDataStatus) {
                $oldSnapshot['status'] = $lastLogDataStatus;
            }
        }
        ...
        if (strlen($status)) {
            ...
            if ($oldStatus && $oldStatus !== $status) {
                $data['status']['old_value'] = $oldStatus;
            }

            $data['status']['new_value'] = $status;
        }
