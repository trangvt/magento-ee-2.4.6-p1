 ...
 */
class SnapshotInformationManagementTest extends TestCase
{
    /**
    ...
    protected function setUp(): void
    {
        ...
        $this->snapshotInformationManagement = $objectManager->getObject(
            SnapshotInformationManagement::class,
            [
    ...
    {
        $expectedResult = [
            'expiration_date' => Expiration::DATE_QUOTE_NEVER_EXPIRES,
            'price' => [
                'type' => 'sample_type',
                'value' => 'sample_value',
            ],
        ...
        $rateCode = 'rate_code';
        $address->expects($this->atLeastOnce())->method('getShippingMethod')->willReturnCallback(
    ...
    {
        $cartItemCollection = $this->getMockBuilder(AbstractCollection::class)
            ->disableOriginalConstructor()
    ...
    public function testPrepareSystemLogData()
    {
        $data = [
            'status' => [
                'new_value' => NegotiableQuoteInterface::STATUS_EXPIRED,
            ],
            'negotiated_price' => 3,
            'shipping' => 2,
            'subtotal' => 5,
