...
use Magento\PurchaseOrder\Model\Customer\Authorization\ApproveRejectPurchaseOrder;
 ...
 */
class ApproveRejectPurchaseOrderPlugin
{
    /**
    ...
    /**
     * After ApproveRejectPurchaseOrder::isAllowed; further restrict to company admin and remaining rule approvers
     *
     * @param ApproveRejectPurchaseOrder $subject
     * @param bool $result
     ...
     */
    public function afterIsAllowed(ApproveRejectPurchaseOrder $subject, $result, $purchaseOrder)
    {
        ...
        $hasUnapprovedRuleForRole = false;
            ...
            foreach ($approvers->getItems() as $approver) {
                if ($approver->getStatus() === AppliedRuleApproverInterface::STATUS_PENDING &&
                    $this->userCanApprove(
                        $approver,
                        (int)$role->getRoleId(),
                        (string)$purchaseOrder->getCreatorId()
                    )
                ) {
                    $hasUnapprovedRuleForRole = true;
                    break;
                }
            }
            if ($hasUnapprovedRuleForRole) {
                break;
            }
        }
        // if results then return true
        return $hasUnapprovedRuleForRole;
    }
     ...
     * @throws LocalizedException
     */
    private function userCanApprove(
        ...
        if ($approverType === AppliedRuleApproverInterface::APPROVER_TYPE_ROLE) {
            return ($approver->getRoleId() == $roleId) && ($creatorId != $this->companyContext->getCustomerId());
