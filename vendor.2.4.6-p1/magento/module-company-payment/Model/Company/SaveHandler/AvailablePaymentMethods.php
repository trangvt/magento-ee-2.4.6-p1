...
/**
 * Plugin for company  AvailablePaymentMethods.
 *
 * Save and process Company available and applicable payment methods
 */
class AvailablePaymentMethods implements \Magento\Company\Model\SaveHandlerInterface
{
    ...
    private function savePaymentSettings(\Magento\Company\Api\Data\CompanyInterface $company)
    {
        ...
        $availablePaymentMethods = $extensionAttributes->getAvailablePaymentMethods();
        $availableMethods = is_array($availablePaymentMethods) ?
            implode(',', $availablePaymentMethods)
            : (!empty($availablePaymentMethods) ? $availablePaymentMethods : '');
