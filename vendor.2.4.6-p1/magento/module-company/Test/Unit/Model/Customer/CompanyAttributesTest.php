...
{
    /**
     * @var \Magento\Company\Model\ResourceModel\Customer|MockObject
     */
    ...
    /**
     * @var AttributesSaver|MockObject
     */
    ...
    protected function setUp(): void
    {
        ...
        $this->extensionFactory = $this->getMockBuilder(
            ...
            ->setMethods(['create'])
            ->getMock();
        $this->attributesSaver = $this->getMockBuilder(AttributesSaver::class)
            ->disableOriginalConstructor()
    ...
    public function testUpdateCompanyAttributes(
        ...
        $customerId = 1;
        $companyStatus = CompanyInterface::STATUS_APPROVED;
    ...
    private function prepareCustomerForUpdateCompanyAttributesWithCompanyChange(
        ...
        $companyAttributes = $this->getMockBuilder(CompanyCustomerInterface::class)
            ->disableOriginalConstructor()
            ->getMockForAbstractClass();
        $originalCompanyAttributes = $this->getMockBuilder(CompanyCustomerInterface::class)
            ->disableOriginalConstructor()
            ->getMockForAbstractClass();
        $customer->expects($this->atLeastOnce())->method('getId')->willReturn($customerId);
        $customer->expects($this->atLeastOnce())
    ...
    public function testSaveCompanyAttributes()
    {
        $companyAttributes = $this->getMockBuilder(CompanyCustomerInterface::class)
            ->disableOriginalConstructor()
            ->getMockForAbstractClass();
        $reflectionClass = new \ReflectionClass(CompanyAttributes::class);
    ...
    {
        $this->expectException('Magento\Framework\Exception\CouldNotSaveException');
        $this->expectExceptionMessage('Invalid attribute value. Cannot change company for a company admin.');
        $customerId = 1;
        $companyId = 2;
        $oldCompanyId = 3;
        $companyStatus = CompanyInterface::STATUS_APPROVED;
    ...
    {
        $this->expectException('Magento\Framework\Exception\CouldNotSaveException');
        $this->expectExceptionMessage('You cannot update the requested attribute. Row ID: companyId = 0.');
        $customerId = 1;
        $customer = $this->getMockBuilder(CustomerInterface::class)
            ->disableOriginalConstructor()
            ->getMockForAbstractClass();
