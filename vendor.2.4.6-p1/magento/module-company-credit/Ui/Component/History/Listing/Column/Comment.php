...
class Comment extends Column
{
     ...
     * @return string
     */
    private function getCommentLabel($serialized)
    {
        $comment = '';
        if (!empty($serialized)) {
            ...
            if (!empty($commentUnserialize['system'])) {
                $commentArray += $this->renderSystemComments($commentUnserialize['system']);
            }
            if (!empty($commentUnserialize['custom'])) {
                array_unshift(
                    $commentArray,
                    $this->escaper->escapeHtml($commentUnserialize['custom'])
                );
            }
     ...
     * @return string[]
     */
    private function renderSystemComments(array $comments)
    {
        $renderedComments = [];
